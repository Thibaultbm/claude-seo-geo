...
- 4.2 Core Web Vitals: LCP under 2.5 s, INP under 200 ms, CLS under 0.1. INP replaced FID in March 2024 and is the most commonly failed vital. Treat CWV as a tiebreaker, not the main lever: fix indexation and content first. (measured)
- 4.3 Sitemap.xml: present, declared in robots.txt, segmented by type for large sites, honest lastmod. Hard limit of 50,000 URLs and 50 MB uncompressed per file: an oversized file is rejected by Google entirely, so sites above the limit must split into child sitemaps under a sitemap index. The Google sitemap ping endpoint is dead since 2023; submission happens in Search Console. (measured)
- 4.4 robots.txt and AI bots: report blocked AI SEARCH bots (OAI-SearchBot, Claude-SearchBot, PerplexityBot) separately from blocked TRAINING bots (GPTBot, ClaudeBot, CCBot, Google-Extended, and others), and treat USER-FETCH agents (ChatGPT-User, Claude-User, Perplexity-User) as a third group: they fetch on a user's explicit request, and robots.txt is not a reliable control for ChatGPT-User and Perplexity-User, so never report a Disallow on them as effective. Blocking search bots removes the site from AI answers today; blocking training bots is a brand-knowledge tradeoff. Default for visibility-seeking businesses: allow all groups. Full table: seo-technical references/ai-crawlers.md. (measured)
