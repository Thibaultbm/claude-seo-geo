---
name: seo-local
description: Improves local search visibility end to end, Google Business Profile (GBP) audit and completion, category strategy, review generation and replies, NAP consistency, location pages with LocalBusiness schema, multi-location and service-area setups, suspended listing reinstatement, and a GEO layer so AI assistants recommend the business for "best [trade] in [city]" queries. Use this skill whenever the user mentions Google Business Profile, GBP, Google Maps, the local pack or 3-pack, local SEO, citations, reviews and star ratings, a restaurant, clinic, law firm, plumber, dentist, gym, or store serving a local area, a business that does not show up on Maps, choosing GBP categories, replying to a negative review, or a suspended listing. Also use it to plan the local page for each branch or service area. For building citation and directory links, use the seo-backlinks skill. For writing the service and city pages themselves, use the seo-content-service-page skill.
license: MIT
metadata:
